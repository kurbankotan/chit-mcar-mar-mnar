...
print("HDD yükleniyor...")
df_hdd = load_hdd()
print(f"Shape: {df_hdd.shape}, class: {df_hdd['class'].value_counts().to_dict()}")

scaler_hdd = StandardScaler(); scaler_hdd.fit(df_hdd.drop('class', axis=1))

np.random.seed(42); tmp=df_hdd.copy(); mask=np.random.rand(*tmp.shape)<0.1
mask[:,tmp.columns.get_loc('class')]=False; tmp[mask]=np.nan
mcar_rows_hdd = np.where(tmp['trestbps'].isnull() & df_hdd['trestbps'].notnull())[0]
df_hdd_mcar = sim_mcar(df_hdd, rate=0.1)
res_hdd_mcar = run_experiment(df_hdd_mcar,'HDD_MCAR',df_hdd,scaler_hdd,'trestbps',mcar_rows_hdd)
...
mar_rows_hdd = np.where(df_hdd_mar['trestbps'].isnull() & df_hdd['trestbps'].notnull())[0]
res_hdd_mar = run_experiment(df_hdd_mar,'HDD_MAR',df_hdd,scaler_hdd,'trestbps',mar_rows_hdd)
del df_hdd_mar; gc.collect()

df_hdd_mnar = df_hdd.copy()
df_hdd_mnar.loc[df_hdd_mnar['chol'] > 250, 'chol'] = np.nan
mnar_rows_hdd = np.where(df_hdd_mnar['chol'].isnull() & df_hdd['chol'].notnull())[0]
res_hdd_mnar = run_experiment(df_hdd_mnar,'HDD_MNAR',df_hdd,scaler_hdd,'chol',mnar_rows_hdd)
del df_hdd_mnar; gc.collect()

print("\n=== HDD TAMAMLANDI ===")
clf_names=['KNN','LR','SVC','DT','RFC','GNB','MLP','DL']
for mech,res in [('HDD_MCAR',res_hdd_mcar),('HDD_MAR',res_hdd_mar),('HDD_MNAR',res_hdd_mnar)]:
...
for fname in ['chit_complete_hdd_mcar.json','chit_complete_hdd_mar.json','chit_complete_hdd_mnar.json']:
